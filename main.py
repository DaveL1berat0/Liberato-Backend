...
async def _refresh_gex_ndx(asset="NQ"):
    ...
    async with httpx.AsyncClient(timeout=12,
        ...
        # (La lista de /options puede incluir fechas sin datos GEX desglosados.)
        future_list = sorted([d for d in exp_dates if d > _today_et_str()])
...
@app.get("/")
def root():
    return {"status":"ok","version":"3.0-FIX31","engine":"TwelveData Realtime + Finnhub + FlashAlpha"}
