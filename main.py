...
FLASHALPHA_KEY_BACKUP = os.getenv("flashback",        "").strip()
...
async def refresh_gex(asset="NQ"):
    ...
    keys_to_try = [k for k in [FLASHALPHA_KEY, FLASHALPHA_KEY_BACKUP] if k]
    last_error = None
    for fa_key in keys_to_try:
      try:
        async with httpx.AsyncClient(timeout=12,
                                      headers={"X-Api-Key": fa_key}) as client:
            r = await client.get(f"{FA_BASE}/v1/stock/{ticker}/summary")
        if r.status_code == 200:
            d = r.json()
            ex = d.get("exposure", {}) or {}
            px = d.get("price",    {}) or {}
            fl = d.get("options_flow", {}) or {}
            qs = d.get("quick_stats",  {}) or {}
            underlying = px.get("mid") or px.get("last")
            ratio = 41.2
            # expected_move: try quick_stats first, fallback calculate from IV
            em_raw = qs.get("expected_move_1d") or qs.get("expected_move")
            # atm_iv and p_c_ratio
            atm_iv = d.get("atm_iv") or qs.get("atm_iv") or ex.get("atm_iv")
            pc_ratio = fl.get("p_c_ratio") or fl.get("pc_ratio_oi") or d.get("p_c_ratio")
            call_vol = fl.get("call_volume")
            put_vol  = fl.get("put_volume")
            cache["gex"][asset] = {
                ...
                "ticker":           ticker,
                "_ts":              time.time(),
                "_key_used":        "primary" if fa_key == FLASHALPHA_KEY else "backup",
            }
            cache["health"]["flashalpha"] = "online"
            save_cache()
            print(f"[gex] ok via {'primary' if fa_key==FLASHALPHA_KEY else 'BACKUP'}: flip={cache['gex'][asset].get('gamma_flip')}")
            break  # success — stop trying keys
        elif r.status_code == 429:
            _gex_blocked_until = time.time() + 86400
            cache["health"]["flashalpha"] = "rate-limited-24h"
            print(f"[gex] 429 on {'primary' if fa_key==FLASHALPHA_KEY else 'backup'} — trying next key")
            last_error = "429"
            continue  # try backup key
        else:
            cache["health"]["flashalpha"] = f"error-{r.status_code}"
            print(f"[gex] error {r.status_code} on {'primary' if fa_key==FLASHALPHA_KEY else 'backup'}")
            last_error = f"error-{r.status_code}"
      except Exception as e:
        cache["health"]["flashalpha"] = "error"
        print(f"[gex] excepción key={'primary' if fa_key==FLASHALPHA_KEY else 'backup'}: {e}")
        last_error = str(e)
...
async def get_nq_price():
    """Precio NQ en tiempo real desde cache TwelveData WS. Sin créditos."""
    ...
    return {
        ...
        "source": "websocket" if nq.get("price") else "rest",
