...
async def get_company(ticker: str):
    ...
    # Enrich with next earnings data:
    # 1. Try our cache (45-day window already loaded)
    next_earn = next(
        ...
        try:
            async with httpx.AsyncClient(timeout=5) as _ec:
                ...
                if re_.status_code == 200:
                    ...
                    if estimates:
                        # First future estimate (period > today)
                        from datetime import date as _date
                        today_str = _date.today().isoformat()
                        future = [e for e in estimates if e.get("period","") > today_str]
                        if future:
                            nxt = future[0]
                            data["nextEpsEstimate"] = nxt.get("epsAvg") or nxt.get("epsMean")
