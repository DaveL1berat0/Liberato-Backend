"""Liberato Backend v2.2 — Parche Anti-Bloqueo Yahoo e Inyección de Claves
"""
import os
import time
import asyncio
    ...
    try:
        async with httpx.AsyncClient(timeout=10, headers={"X-Api-Key": FLASHALPHA_KEY}) as client:
            ...
            if r.status_code == 200:
                ...
                cache["health"]["flashalpha"] = {"status": "error", "reason": f"FlashAlpha HTTP {r.status_code}. Clave inválida."}
    except Exception as e:
    ...
    try:
        async with httpx.AsyncClient(timeout=10) as client:
            ...
            if r.status_code == 200:
                ...
                cache["health"]["finnhub"] = {"status": "online", "reason": "Calendario sincronizado."}
            else:
...
async def refresh_heatmap():
    try:
        # CORRECCIÓN: Endpoint alternativo institucional de Yahoo sin cookies restrictivas
        url = "https://query2.finance.yahoo.com/v10/finance/quoteSummary/QQQ?modules=price"
        async with httpx.AsyncClient(timeout=10, headers={"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"}) as client:
            r = await client.get(url)
            if r.status_code == 200:
                res = r.json().get("quoteSummary", {}).get("result", [])
                if res:
                    p_data = res[0].get("price", {})
                    cache["heatmap"]["data"] = {
                        "QQQ": {
                            "price": p_data.get("regularMarketPrice", {}).get("raw"),
                            "chg_pct": p_data.get("regularMarketChangePercent", {}).get("raw")
                        }
                    }
                    cache["health"]["yahoo"] = {"status": "online", "reason": "Heatmap sincronizado mediante Query2."}
                    return
                cache["health"]["yahoo"] = {"status": "error", "reason": "Yahoo Query2 no retornó estructura esperada."}
            else:
                cache["health"]["yahoo"] = {"status": "error", "reason": f"Yahoo Query2 HTTP {r.status_code}"}
    except Exception as e:
        cache["health"]["yahoo"] = {"status": "error", "reason": f"Fallo Yahoo Query2: {str(e)}"}
