...
def _nearest_index_expiration():
    ...
    d = now_et.date()
    # weekday(): Mon=0 ... Sun=6. NDX/SPX tienen expiración Lun(0), Mié(2), Vie(4).
    exp_days = {0, 2, 4}
    for i in range(0, 8):
...
async def _refresh_gex_ndx(asset="NQ"):
    ...
    async with httpx.AsyncClient(timeout=12,
        ...
        # IMPORTANTE: en Basic, /gex de índices requiere ?expiration (un solo
        # expiry). Sin él, FlashAlpha asume full-chain → 403 (Growth-tier).
        net_gex = None; per_strike = None
        exp = _nearest_index_expiration()
        try:
            ...
            if r_gex.status_code == 200:
                ...
                print(f"[gex] /gex/{sym} 403 aún con expiration={exp} (¿full-chain?). Usando net_gex de levels si existe.")
            else:
                print(f"[gex] /gex/{sym} status {r_gex.status_code}")
        except Exception as e:
            print(f"[gex] NDX gex secundario falló (no crítico): {e}")
        # Si /gex no dio net_gex, intentar tomarlo de la respuesta de levels
        if net_gex is None:
...
@app.get("/")
def root():
    return {"status":"ok","version":"3.0-FIX25","engine":"TwelveData Realtime + Finnhub + FlashAlpha"}
    ...
    try:
        async with httpx.AsyncClient(timeout=12,
            ...
            if r_lvl.status_code == 200:
                ...
                out["levels_body"] = r_lvl.text[:200]
            # GEX (net + per-strike) — con expiración (requerido en Basic para índices)
            exp = _nearest_index_expiration()
            out["expiration_usada"] = exp
            r_gex = await client.get(f"{FA_BASE}/v1/exposure/gex/{sym}",
                                     params={"expiration": exp})
            ...
            if r_gex.status_code == 200:
                ...
                out["gex_interpretacion"] = "✅ /gex con expiración FUNCIONA en Basic"
            elif r_gex.status_code == 403:
                out["gex_body"] = r_gex.text[:200]
                out["gex_interpretacion"] = "⚠️ /gex sigue 403 — revisar formato de expiración"
            else:
                out["gex_body"] = r_gex.text[:200]
    except Exception as e:
        out["error"] = str(e)
