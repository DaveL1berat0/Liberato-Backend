...
async def _fetch_rapidapi_actuals(client):
    ...
    try:
        ...
        endpoints = ["/", "/economic-events", "/calendar", "/events"]
        r = None
        for ep in endpoints:
            try:
...
async def refresh_calendar():
    ...
    async def _fetch_fmp(client):
        ...
        try:
            ...
            r = await client.get(f"{FMP_BASE}/economic_calendar",
...
@app.get("/")
def root():
    return {"status":"ok","version":"3.0-FIX16","engine":"TwelveData Realtime + Finnhub + FlashAlpha"}
