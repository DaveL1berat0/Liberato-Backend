...
async def _market_candles_impl(tf: str = "5"):
    ...
    if c5 and (time.time() - c5["ts"]) < 90 and c5["data"].get("candles"):
        ...
        if TWELVEDATA_KEY:
            base = await _fetch_td_5m_base()      # 1) TwelveData (si hay key+presupuesto)
        if not (base and base.get("candles")):
            base = await _fetch_yahoo_5m_base()   # 2) Yahoo (gratis, sin key ni límite)
...
async def startup():
    ...
    # ── Calentar velas del chart cada 4 min (TwelveData→Yahoo→cache) ────────────
    # Garantiza que /api/market/candles siempre tenga base 5m lista para servir.
    async def _warm_candles():
        try: await _market_candles_impl("5")
        except Exception as e: print(f"[candles] warm error: {e}")
    scheduler.add_job(_warm_candles, IntervalTrigger(minutes=4))
