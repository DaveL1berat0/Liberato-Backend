...
async def get_company(ticker: str):
    ...
    # Enrich with next earnings data from our Finnhub calendar cache
    # (1,497 upcoming earnings already loaded — most reliable free source)
    all_upcoming = [e for e in cache["earnings"]["data"]
                    if e.get("symbol","").upper() == sym and not e.get("epsActual")]
    all_upcoming.sort(key=lambda e: e.get("date",""))  # earliest first
    ...
    # 2. If no estimate yet, fetch directly from Finnhub earnings estimate endpoint
    #    This covers companies whose next report is >45 days away
    if FINNHUB_KEY and not data.get("nextEpsEstimate"):
        try:
            async with httpx.AsyncClient(timeout=5) as _ec:
                # Finnhub EPS estimates — returns consensus for next quarters
                re_ = await _ec.get(f"{FH_BASE}/stock/eps-estimate",
                                    params={"symbol": sym, "freq": "quarterly",
                                            "token": FINNHUB_KEY})
                if re_.status_code == 200:
                    est_data = re_.json() or {}
                    estimates = est_data.get("data") or []
                    if estimates:
                        from datetime import date as _date
                        today_str = _date.today().isoformat()
                        # Sort by period ascending, take first future quarter
                        estimates_sorted = sorted(estimates, key=lambda e: e.get("period",""))
                        future = [e for e in estimates_sorted
                                  if str(e.get("period","")) >= today_str[:7]]  # YYYY-MM match
                        if future:
                            nxt    = future[0]
                            eps_v  = (nxt.get("epsAvg") or nxt.get("epsMean")
                                      or nxt.get("epsEstimate") or nxt.get("estimate"))
                            if eps_v is not None:
                                data["nextEpsEstimate"] = round(float(eps_v), 2)
                                data["nextDate"]        = nxt.get("period")
        except Exception as _ee:
            pass  # silent — EPS estimate is best-effort
