...
async def refresh_gex(asset="NQ"):
    ...
    try:
        async with httpx.AsyncClient(timeout=12,
            ...
            # Usar el summary (trae niveles + precio + as_of en una sola llamada).
            r = await client.get(f"{FA_BASE}/v1/stock/{ticker}/summary")
        if r.status_code == 200:
            d = r.json()
            ex = d.get("exposure", {}) or {}
            px = d.get("price",    {}) or {}
            # Extraer niveles. FlashAlpha summary los da como NÚMEROS directos,
            # pero por robustez manejamos también el caso objeto {strike, gex}.
            def _lvl(v):
            ...
            # as_of de FlashAlpha = cuándo se CALCULÓ el dato (no cuándo llamamos)
            as_of = d.get("as_of")
            ...
            cache["gex"][asset] = {
                ...
                "ticker":     ticker,
                "as_of":      as_of,           # timestamp REAL del cálculo
                "market_open": market_open,
                "_ts":        time.time(),     # cuándo llamamos nosotros
            }
            # Validar que los niveles llegaron (no None) — si no, registrar advertencia
            if cw is None and pw is None and gf is None:
                cache["health"]["flashalpha"] = "online-no-levels"
                print(f"[gex] ⚠️ FlashAlpha respondió 200 pero SIN niveles (¿plan no cubre QQQ?). Keys: {list(ex.keys())}")
            else:
                cache["health"]["flashalpha"] = "online"
                print(f"[gex] ok: flip={gf} call={cw} put={pw} as_of={as_of}")
            save_cache()
...
_ff_cache = []        # último resultado bueno de ForexFactory (límite 2/5min)
...
async def _fetch_rapidapi_actuals(client):
    ...
    h = now_et.hour
    m = now_et.minute
    # 8-11am completo + solo 2:00-2:30pm (FOMC sale a las 2:00 exactas)
    in_market_hours = (8 <= h < 11) or (h == 14 and m < 30)
    if not (is_weekday and in_market_hours):
        return []  # fuera de horario → ForexFactory + Finnhub son suficientes
    # Guard de frecuencia: RapidAPI actualiza cada 5 min, así que no tiene
    # sentido llamarla más seguido. Esto respeta el límite del plan gratuito
    # (1,000/mes) llamando solo cada 5 min en vez de cada 60s.
    global _rapidapi_last_call
    nowts = time.time()
    if nowts - _rapidapi_last_call < 300:  # 300s = 5 min
...
async def refresh_calendar():
    ...
    FF_URLS = [
        # ForexFactory limita a 2 descargas cada 5 min (todas las URLs juntas).
        # Por eso usamos SOLO 2 URLs: esta semana + próxima semana de un dominio.
    ...
    async with httpx.AsyncClient(headers=FF_HEADERS, follow_redirects=True) as client:
        ...
        global _ff_last_fetch, _ff_cache
        nowts = time.time()
        fetch_ff_now = (nowts - _ff_last_fetch >= 180)  # 3 min
        if fetch_ff_now:
            _ff_last_fetch = nowts
            ff_tasks = [_fetch_ff(client, url) for url in FF_URLS]
        else:
            ff_tasks = []  # no descargar FF este ciclo (usar cache)
        # Finnhub + RapidAPI siempre corren
        other_tasks = [_fetch_finnhub_fallback(client), _fetch_rapidapi_actuals(client)]
        all_results = await asyncio.gather(*(ff_tasks + other_tasks), return_exceptions=True)
        # Separar resultados de forma robusta
        n_ff = len(ff_tasks)
        ff_fresh = []
        for i in range(n_ff):
            if isinstance(all_results[i], list):
                ff_fresh.extend([e for e in all_results[i] if e])
        fh_events_raw = all_results[n_ff]   if len(all_results) > n_ff   else []
        rt_actuals_raw = all_results[n_ff+1] if len(all_results) > n_ff+1 else []
        # Cachear FF si trajo datos frescos; si no, usar lo cacheado
        if ff_fresh:
            _ff_cache = ff_fresh
        ff_events = list(_ff_cache)  # usar cache (recién actualizado o anterior)
        fh_events = [e for e in fh_events_raw if e] if isinstance(fh_events_raw, list) else []
        rt_actuals = [e for e in rt_actuals_raw if e] if isinstance(rt_actuals_raw, list) else []
        ...
        # ── CAPA TIEMPO REAL: fusionar el "actual" de RapidAPI ──
        # Rellena el resultado de eventos (ADP, CPI, NFP) a los minutos del
        # release, cuando ForexFactory aún no lo tiene.
        if rt_actuals:
...
@app.get("/")
def root():
    return {"status":"ok","version":"3.0-FIX15","engine":"TwelveData Realtime + Finnhub + FlashAlpha"}
...
async def gamma_levels():
    ...
    if not ratio:
        # Respaldo: calcular desde los precios reales del heatmap (NQ / QQQ)
        try:
            hm = cache["heatmap"]["data"]
            ...
            if nq_p and qqq_p and qqq_p > 100:
                ...
                print(f"[ratio] calculado del heatmap: {ratio}")
        except Exception:
            pass
    ratio = ratio or 41.51
    nq  = round(qqq*ratio,2) if qqq else None
    # Convertir los niveles GEX de escala QQQ a escala NQ (multiplicar por el ratio).
    # FlashAlpha devuelve los niveles en QQQ (~725); el gráfico usa NQ (~30,100).
    def _to_nq(v):
...
async def startup():
    ...
    # FlashAlpha — 4 ventanas de análisis estratégicas (4 créditos/día):
    scheduler.add_job(refresh_gex, CronTrigger(hour=19, minute=0,  day_of_week="mon-fri"))  # post-market, prep siguiente sesión
    scheduler.add_job(refresh_gex, CronTrigger(hour=9,  minute=0,  day_of_week="mon-fri"))  # contexto premarket
    scheduler.add_job(refresh_gex, CronTrigger(hour=9,  minute=15, day_of_week="mon-fri"))  # validación final pre-apertura
    scheduler.add_job(refresh_gex, CronTrigger(hour=9,  minute=45, day_of_week="mon-fri"))  # flujo institucional tras primeros 15min
...
async def manual_refresh_gex(key: str = ""):
    ...
    try:
        ...
        if gex:
            ...
            return {
                "success": True,
                "message": "FlashAlpha llamado manualmente ✓",
