...
_gex_daily_count   = 0   # llamadas realizadas hoy
_gex_last_date     = ""  # fecha ET del último reset (YYYY-MM-DD)

# Horarios exactos de llamada FlashAlpha (hora ET, minuto ET)
GEX_SCHEDULE = [
    (19, 0),   # 1. Cierre de mercado
    (9,  0),   # 2. Pre-Market
    (9, 15),   # 3. Ajuste previo
    (9, 30),   # 4. Apertura RTH
    (9, 45),   # 5. Estabilización
]
GEX_MAX_CALLS = 5  # máximo absoluto por día calendario

async def refresh_gex(asset="NQ"):
    """GEX desde FlashAlpha.
    Solo se ejecuta en los 5 minutos exactos del GEX_SCHEDULE.
    Fuera de esos momentos, devuelve caché sin llamar a la API.
    Contador diario: máximo 5 llamadas. Reset a medianoche ET."""
    global _gex_blocked_until, _gex_daily_count, _gex_last_date

    # ── Reset contador diario a medianoche ET ─────────────────────────────
    today_et = datetime.now(NY).strftime("%Y-%m-%d")
    if today_et != _gex_last_date:
        _gex_daily_count = 0
        _gex_last_date   = today_et
        print(f"[gex] contador diario reseteado para {today_et}")

    # ── Bloqueo por contador: máximo GEX_MAX_CALLS por día ───────────────
    if _gex_daily_count >= GEX_MAX_CALLS:
        print(f"[gex] límite diario de {GEX_MAX_CALLS} llamadas alcanzado — sirviendo caché")
        cache["health"]["flashalpha"] = f"daily-limit-{_gex_daily_count}/{GEX_MAX_CALLS}"
        return

    # ── Bloqueo horario: SOLO se ejecuta en los minutos del GEX_SCHEDULE ─
    now_et = datetime.now(NY)
    current_hm = (now_et.hour, now_et.minute)
    if current_hm not in GEX_SCHEDULE:
        # Fuera del horario exacto → servir caché silenciosamente
        if cache["gex"].get(asset):
            cache["health"]["flashalpha"] = f"cached-ok ({_gex_daily_count}/{GEX_MAX_CALLS} calls today)"
        else:
            cache["health"]["flashalpha"] = "waiting-for-schedule"
        return

    # ── Verificaciones de key y bloqueo por 429 ───────────────────────────
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            _gex_daily_count += 1
            save_cache()
            print(f"[gex] ok ({_gex_daily_count}/{GEX_MAX_CALLS} hoy): flip={cache['gex'][asset].get('gamma_flip')}")
...
async def gamma_levels():
    """GEX desde cache. Nunca lanza 503 — devuelve última lectura disponible.
    FlashAlpha se llama según GEX_SCHEDULE (máx 5/día)."""
    gex = cache["gex"].get("NQ")
    if not gex:
        # Sin datos en caché — devolver estructura vacía con metadata
        return {
            "asset": "NQ", "ratio": 41.2,
            "call_wall": None, "put_wall": None, "gamma_flip": None,
            "net_gex": None, "regime": None, "nq_price": None,
            "status": "waiting-for-schedule",
            "next_update": f"Próxima llamada según GEX_SCHEDULE ({GEX_MAX_CALLS} max/día)",
            "daily_calls": f"{_gex_daily_count}/{GEX_MAX_CALLS}",
            "credits_used": _gex_daily_count,
        }
    qqq = gex.get("underlying_price")
    nq  = round(qqq*41.2, 2) if qqq else None
    # NQ price en tiempo real desde TwelveData WS (siempre disponible)
    nq_live = cache["heatmap"]["data"].get("NQ", {}).get("price")
    return {
        **gex,
        "asset": "NQ",
        "nq_price": nq_live or nq,   # prioridad: WS en vivo
        "nq_price_source": "websocket" if nq_live else "flashalpha",
        "ratio": 41.2,
        "credits_used": _gex_daily_count,
        "daily_limit": GEX_MAX_CALLS,
        "cached_at": gex.get("_ts"),
    }
...
@app.get("/api/price/NQ")
async def get_nq_price():
    """Precio NQ en tiempo real desde TwelveData WS. Sin créditos. 24/7."""
    nq = cache["heatmap"]["data"].get("NQ", {})
    qqq = cache["heatmap"]["data"].get("QQQ", {})
    # Prioridad: NQ directo (QQQ×41.2 del WS) → calcular desde QQQ → None
    price = nq.get("price")
    source = "websocket-nq"
    if not price and qqq.get("price"):
        price = round(qqq["price"] * 41.2, 2)
        source = "websocket-qqq-converted"
    return {
        "price":     price,
        "chg_pct":   nq.get("chg_pct"),
        "direction": nq.get("direction"),
        "source":    source,
        "ts":        cache["heatmap"].get("last_update"),
        "status":    cache["heatmap"].get("status", "unknown"),
    }
...
async def startup():
    ...
    # ── TwelveData REST: batch 13 símbolos macro cada 15min — 24/7 ─────────
    # Precio NQ via WS (QQQ×41.2) es siempre tiempo real; REST actualiza los demás
    scheduler.add_job(refresh_heatmap_rest, IntervalTrigger(minutes=15))

    # ── FlashAlpha GEX: 5 horarios exactos — contador interno limita a 5/día
    # 19:00 (cierre), 09:00 (pre-mkt), 09:15 (ajuste), 09:30 (apertura), 09:45 (estabilización)
    ...
    # ── GEX: cargar desde disco O forzar llamada inmediata si no hay caché ─
    if cache["gex"].get("NQ"):
        print("[startup] GEX cargado desde disco ✓")
        asyncio.create_task(refresh_institutional())
    else:
        print("[startup] Sin GEX en disco — forzando llamada inmediata a FlashAlpha")
        # Override de horario para la primera carga en startup
        global _gex_last_date, _gex_daily_count
        now_et = datetime.now(NY)
        # Temporalmente añadir el momento actual al schedule para que pase el guard
        GEX_SCHEDULE.append((now_et.hour, now_et.minute))
        await refresh_gex("NQ")
        # Remover el entry temporal
        try:
            GEX_SCHEDULE.remove((now_et.hour, now_et.minute))
        except ValueError:
            pass
        if cache["gex"].get("NQ"):
            print("[startup] GEX obtenido en startup ✓")
            asyncio.create_task(refresh_institutional())
        else:
            print("[startup] GEX no disponible — mostrará caché vacía hasta próximo schedule")
