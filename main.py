...
# Símbolos Yahoo Finance — todos ETFs estándar (más estables que futuros/índices raw)
# Los proxies reflejan el mismo movimiento que el instrumento original
HMAP_TICKERS = {
    # Top 10 Nasdaq por peso (tickers directos, siempre funcionan)
    "AAPL":"AAPL",  "MSFT":"MSFT",  "NVDA":"NVDA",  "GOOGL":"GOOGL",
    "AMZN":"AMZN",  "META":"META",  "AVGO":"AVGO",  "TSLA":"TSLA",
    "COST":"COST",  "NFLX":"NFLX",
    # ETFs de referencia (proxies de futuros — más confiables en Yahoo)
    "NQ":  "QQQ",   # Nasdaq-100 ETF ≈ NQ Futures
    "ES":  "SPY",   # S&P 500 ETF  ≈ ES Futures
    # Macro — ETFs de volatilidad, divisas, yields, commodities
    "VIX": "VIXY",  # ProShares VIX ETF (vs raw ^VIX que falla en Railway)
    "DXY": "UUP",   # Invesco Dollar Bullish ETF (vs DX-Y.NYB)
    "US2Y":"SHY",   # iShares 1-3Y Treasury ETF (proxy 2Y yield)
    "US10Y":"IEF",  # iShares 7-10Y Treasury ETF (proxy 10Y yield)
    "US30Y":"TLT",  # iShares 20Y+ Treasury ETF  (proxy 30Y yield)
    "Gold":"GLD",   # SPDR Gold ETF (vs GC=F futures)
    "WTI": "USO",   # United States Oil ETF (vs CL=F futures)
    "BTC": "IBIT",  # iShares Bitcoin Trust ETF (vs BTC-USD que a veces falla)
    "INFL":"TIP",   # iShares TIPS ETF — 10Y Breakeven Inflation proxy
}
...
async def fetch_heatmap():
    ...
    if len(out) < 3:
        raise RuntimeError(f"Yahoo v7 retorno solo {len(out)} activos — usando fallback")
    return out
...
async def fetch_heatmap_fallback():
    ...
    if len(results) < 3:
        raise RuntimeError(f"Heatmap fallback: solo {len(results)} activos. Railway puede estar bloqueado por Yahoo.")
    return results
