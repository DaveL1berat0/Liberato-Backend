...
ANTHROPIC_KEY  = os.getenv("ANTHROPIC_KEY", "")         # NUEVO v2 — Claude Haiku
...
def health():
    return {
        ...
        "anthropic":            cache["health"]["anthropic"],  # NUEVO v2
        ...
        "institutional_status": cache["institutional"]["status"],  # NUEVO v2
    }
...
async def fetch_institutional_summary() -> str:
    if not ANTHROPIC_KEY:
        raise RuntimeError("Sin ANTHROPIC_KEY")
    ...
    if call_wall and put_wall and gamma_flip:
        ...
        ctx.append(f"- Gamma: Call Wall {call_wall:.0f} | Put Wall {put_wall:.0f} | Flip {gamma_flip:.0f} | NQ ~{nq_approx:.0f} ({pdir} flip)")
        ctx.append(f"- Régimen dealer: {regime} | Net GEX: {net_gex}")
    if vix.get("chg_pct"):
        vdir = "cayendo" if vix["direction"] == "down" else "subiendo"
        ctx.append(f"- VIX: {vdir} {vix['chg_pct']:+.1f}%")
    if dxy.get("chg_pct"):  ctx.append(f"- DXY: {dxy['chg_pct']:+.1f}%")
    if us10y.get("chg_pct"):ctx.append(f"- US10Y: {us10y['chg_pct']:+.1f}%")
    if nvda.get("chg_pct"): ctx.append(f"- NVDA: {nvda['chg_pct']:+.1f}%")
    if next_event: ctx.append(f"- Próximo macro: {next_event}")
    if earnings_today: ctx.append(f"- Earnings hoy: {', '.join(earnings_today)}")
    context_str = "\n".join(ctx) if ctx else "Datos insuficientes"
    prompt = f"""Eres el analista institucional de Liberato Community.
Genera un briefing en ESPAÑOL con exactamente 2-3 oraciones.

DATOS:
{context_str}

REGLAS ESTRICTAS:
- Máximo 3 oraciones, nunca listas ni bullets.
- Menciona los niveles exactos de gamma con los números reales.
- Explica el sesgo institucional y qué significa para el trader.
- Si hay catalizadores (earnings, macro importante) mencionarlo al final.
- Lenguaje institucional: "dealers mantienen", "gamma comprime", "sesgo constructivo/bajista".
- Nunca promesas de retorno ni recomendaciones de compra/venta directa."""
    ...
    if r.status_code != 200:
        raise RuntimeError(f"Anthropic {r.status_code}")
    text = next((b["text"] for b in r.json().get("content", [])
                 if b.get("type") == "text"), None)
    if not text: raise RuntimeError("Anthropic respuesta vacía")
    return text.strip()
...
async def refresh_institutional_summary():
    try:
        ...
        cache["health"]["anthropic"]     = "stale" if cache["institutional"]["text"] else "offline"
