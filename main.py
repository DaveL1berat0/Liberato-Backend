...
NUEVO v2: /api/dashboard + /api/heatmap + /api/context/institutional + WebSockets Reales
"""
import os
import time
import asyncio
...
async def refresh_gex(asset="NQ"):
    ...
    try:
        ...
        print(f"[GEX] Error al refrescar: {e}")
...
async def refresh_calendar():
    if not FINNHUB_KEY:
        ...
        return
    try:
        # Mock o llamada directa a Finnhub para asegurar el online
        ...
        async with httpx.AsyncClient(timeout=20) as client:
            r = await client.post(
                ...
                json={
                    ...
                    "messages": [{"role": "user", "content": f"Genera un resumen breve de trading indicando mercado listo para operar en base a: {context_str}"}]
                }
            )
            if r.status_code == 200:
...
def get_institutional(): return {"summary": cache["institutional"]["text"]}
...
async def startup():
    ...
    # Conexión continua por debajo
    asyncio.create_task(twelvedata_websocket_listener())
    
    # LUEGO DEL CAMBIO: Llamadas independientes y seguras una tras otra
    try: await refresh_gex()
    except Exception: pass
    
    try: await refresh_calendar()
    except Exception: pass
    
    try: await refresh_heatmap()
    except Exception: pass
    
    try: await refresh_institutional_summary()
    except Exception: pass
    
    print("[startup] Backend v2 listo con cargas secuenciales blindadas.")