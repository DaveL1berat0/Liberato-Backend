...
@app.get("/")
def root():
    return {"status":"ok","version":"3.0-FIX14","engine":"TwelveData Realtime + Finnhub + FlashAlpha"}
...
async def gamma_levels():
    ...
    # Timestamp de la última llamada exitosa. Si el GEX viene de disco sin _ts,
    # usar la fecha de modificación del archivo de caché como respaldo.
    ts = gex.get("_ts")
    if not ts:
        try:
