...
API_BUDGETS = {
    ...
    "flashalpha":  {"limit": int(os.getenv("FA_DAILY_LIMIT", "90")),   "window": "day"},   # real 100
...
async def _refresh_gex_ndx(asset="NQ"):
    ...
    # Registrar el costo estimado de este refresh (levels+options+gex+maxpain+summary)
    budget_charge("flashalpha", 5)
    async with httpx.AsyncClient(timeout=12,
                                  headers={"X-Api-Key": FLASHALPHA_KEY}) as client:
        ...
        for cand_exp in future_list[:4]:   # máximo 4 intentos
            try:
                ...
                if r_gex.status_code == 200:
                    ...
                    print(f"[gex] /gex/{sym}?expiration={cand_exp} OK net_gex={net_gex}")
                    break
                else:
    ...
    if mp is None:
        try:
            async with httpx.AsyncClient(timeout=12,
                ...
                r_mp = await mpc.get(f"{FA_BASE}/v1/maxpain/{sym}")
            if r_mp.status_code == 200:
                mpd = r_mp.json() or {}
                mp = _num(mpd.get("max_pain") or mpd.get("maxpain") or mpd.get("max_pain_strike"))
                print(f"[gex] max_pain (NDX): {mp}")
            else:
                print(f"[gex] /maxpain/{sym} status {r_mp.status_code}")
        except Exception as e:
    ...
    try:
        async with httpx.AsyncClient(timeout=12,
            ...
            r_sum = await ivc.get(f"{FA_BASE}/v1/stock/{sym}/summary")
...
async def startup():
    ...
    _gex_windows = [
        (7, 0),   # premarket temprano
        (7, 30),  # premarket
        (8, 0),   # antes de datos económicos (8:30)
        (8, 45),  # post-datos económicos
        (9, 15),  # antes de apertura
        (9, 30),  # APERTURA
        (9, 50),  # post-apertura
        (10, 15), # media mañana
        (10, 30), # media mañana
        ...
        (13, 0),  # tarde
        (14, 30), # tarde
        (15, 30), # antes del cierre
        (16, 0),  # cierre: snapshot final
    ]
    for _gh, _gm in _gex_windows:
        scheduler.add_job(refresh_gex,
                          CronTrigger(hour=_gh, minute=_gm, day_of_week="mon-fri"))
