...
def _merge_rapidapi(ff_events, rt_actuals):
    ...
    # (2) Inyectar eventos de TE que FF no tiene (solo high-impact ya publicados)
    for key, rt in rt_index.items():
        if key in ff_keys or not rt.get("actual"):
            continue
        ff_events.append({
            "title": rt["title"], "time": rt.get("date",""), "impact": "high",
            "actual": rt.get("actual"), "forecast": rt.get("consensus"),
            "previous": rt.get("previous"), "status": "Released", "type": "macro",
...
async def _fetch_td_5m_base():
    """Trae la base 5m real de TwelveData (NQ→NDX→QQQ con ratio). 390 barras
    (~5 días de sesión) para que el resampleo a 30m tenga historia suficiente."""
    ratio = cache["nq_ratio"].get("value") or 41.51
    ...
    for sym, mult in candidates:
        ...
        if not td_budget_ok(1):
            print(f"[candles] presupuesto TwelveData agotado ({_td_credits['used']}/{TD_DAILY_LIMIT}) — probando Yahoo")
            return None
        try:
            url = (f"https://api.twelvedata.com/time_series?symbol={sym}"
                   f"&interval=5min&outputsize=390&apikey={TWELVEDATA_KEY}")
            async with httpx.AsyncClient(timeout=15) as client:
                r = await client.get(url)
            ...
            if out:
                ...
                print(f"[candles] base 5m {sym} ok ({len(out)} velas)")
                return {"status":"ok","symbol":sym,"interval":"5min",
                        "candles":out,"source":src,"converted": mult != 1.0}
        except Exception as e:
            print(f"[candles] {sym} 5min falló: {e}")
            continue
    return None
...
async def _market_candles_impl(tf: str = "5"):
    ...
    if cached and (time.time() - cached["ts"]) < 90:
        return cached["data"]
    # ── ARQUITECTURA: TwelveData se consulta SOLO para la base 5m. 15m y 30m
    # se DERIVAN agregando esas velas reales (resampleo OHLC estándar).
    # Beneficios: (1) los 3 timeframes funcionan siempre que exista base 5m —
    # nunca más un TF muerto por su cuenta, (2) ~66% menos créditos TwelveData,
    # (3) los tres TF siempre coherentes entre sí (misma fuente).
    base = None
    c5 = _candles_cache.get("5")
    if c5 and (time.time() - c5["ts"]) < 90 and c5["data"].get("candles"):
        base = c5["data"]
    else:
        base = await _fetch_td_5m_base()          # 1) TwelveData (si hay presupuesto)
        if not (base and base.get("candles")):
            base = await _fetch_yahoo_5m_base()   # 2) Yahoo (gratis, sin límite)
        if base and base.get("candles"):
            _candles_cache["5"] = {"ts": time.time(), "data": base}
        elif c5 and c5["data"].get("candles"):
            base = {**c5["data"], "note": "base-5m-desde-cache"}  # 3) último real
