...
async def _fetch_rapidapi_actuals(client):
    ...
    try:
        ...
        endpoints = ["/", "/economic-calendar", "/economic_events", "/calendar", "/events", "/economic-events"]
        r = None
        for ep in endpoints:
            try:
                url = f"https://{RAPIDAPI_HOST}{ep}"
                rr = await client.get(url, headers=headers, params=params, timeout=10)
                if rr.status_code == 200:
                    r = rr
                    print(f"[rt-calendar] RapidAPI OK en {ep}")
                    break
                elif rr.status_code in (401, 403):
                    print(f"[rt-calendar] RapidAPI {rr.status_code} en {ep}: {rr.text[:100]}")
                    return []  # auth error → no seguir probando
            except Exception:
                continue
        if r is None:
            print(f"[rt-calendar] RapidAPI: ningún endpoint respondió 200")
            return []
        data = r.json()
...
async def refresh_calendar():
    ...
    async with httpx.AsyncClient(headers=FF_HEADERS, follow_redirects=True) as client:
        ...
        # ── FMP: cada 4 min, SOLO en ventana de releases (cuida las 250/día) ──
        fetch_fmp_now = bool(FMP_KEY) and in_release_window and (nowts - _fmp_last_fetch >= 240)
        if fetch_fmp_now:
...
@app.get("/")
def root():
    return {"status":"ok","version":"3.0-FIX20","engine":"TwelveData Realtime + Finnhub + FlashAlpha"}
    ...
    async with httpx.AsyncClient(timeout=15) as client:
        ...
        try:
            if FMP_KEY:
                now_et = datetime.now(NY)
                frm = now_et.strftime("%Y-%m-%d")
                to_diag = (now_et + timedelta(days=2)).strftime("%Y-%m-%d")
                r = await client.get(f"{FMP_BASE}/economic-calendar",
                    params={"from": frm, "to": to_diag, "apikey": FMP_KEY})
...
async def diag_rapidapi(key: str = ""):
    ...
    params = {"from": today.strftime("%Y-%m-%d"),
              "to": (today + timedelta(days=1)).strftime("%Y-%m-%d"), "countries": "US"}
    paths = ["/economic_calendar", "/economicCalendar", "/api/v1/economic-calendar",
             "/v1/economic-calendar", "/v1/calendar", "/api/calendar",
             "/getEconomicCalendar", "/economic-calendar/events", "/data",
             "/api/events", "/economicevents", "/economic", "/v1/events",
