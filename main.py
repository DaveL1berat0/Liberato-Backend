...
async def refresh_calendar():
    ...
    async def _fetch_ff(client, url):
        try:
            # Cache-busting: añadir timestamp para forzar datos frescos del CDN
            # (el CDN puede servir JSON cacheado sin el 'actual' más reciente)
            bust_url = url + ("&" if "?" in url else "?") + "_=" + str(int(time.time()))
            r = await client.get(bust_url, timeout=8)
    ...
    async with httpx.AsyncClient(headers=FF_HEADERS, follow_redirects=True) as client:
        ...
        # Fetch en paralelo: ForexFactory + Finnhub + RapidAPI (tiempo real)
        all_tasks = ff_tasks + [_fetch_finnhub_fallback(client), _fetch_rapidapi_actuals(client)]
        results = await asyncio.gather(*all_tasks, return_exceptions=True)

        ff_events, fh_events, rt_actuals = [], [], []
        for i, res in enumerate(results):
            if isinstance(res, list):
                if i < len(ff_tasks): ff_events.extend([e for e in res if e])
                elif i == len(ff_tasks): fh_events.extend([e for e in res if e])  # Finnhub
                else: rt_actuals.extend([e for e in res if e])  # RapidAPI (último)
...
async def startup():
    ...
    scheduler.add_job(refresh_calendar, IntervalTrigger(seconds=90))
