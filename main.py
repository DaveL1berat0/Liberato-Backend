...
async def get_company(ticker: str):
    ...
    if FINNHUB_KEY:
        async with httpx.AsyncClient(timeout=8) as client:
            # Finnhub profile — name, sector, market cap
            if not data.get("name"):
                try:
                    ...
                    print(f"[company] profile {sym}: {_pe}")

            # Finnhub historical earnings — last 4 quarters
            try:
                rh = await client.get(f"{FH_BASE}/stock/earnings",
                                       params={"symbol":sym,"limit":8,"token":FINNHUB_KEY})
                if rh.status_code == 200:
                    ...
                    if hist:
                        data["history"] = hist
                        # EPS growth from most recent vs a year ago
                        if len(hist) >= 2 and hist[0]["epsActual"] and hist[-1]["epsActual"]:
                            try:
                                g = ((hist[0]["epsActual"]-hist[-1]["epsActual"])
                                     /abs(hist[-1]["epsActual"])*100)
                                data["epsGrowthYoY"] = f"{'+' if g>=0 else ''}{g:.1f}%"
                            except: pass
            except Exception as _he:
                print(f"[company] hist {sym}: {_he}")
            
            # Finnhub metric — EPS growth YoY if not already set
            if not data.get("epsGrowthYoY"):
                try:
                    ...
                    print(f"[company] metric {sym}: {_me}")
