...
async def refresh_calendar():
    ...
    async def _fetch_fmp(client):
        ...
        try:
            ...
            r = await client.get(f"{FMP_BASE}/economics-calendar",
...
@app.get("/")
def root():
    return {"status":"ok","version":"3.0-FIX19","engine":"TwelveData Realtime + Finnhub + FlashAlpha"}
    ...
    async with httpx.AsyncClient(timeout=15) as client:
        ...
        try:
            if FMP_KEY:
                ...
                frm = now_et.strftime("%Y-%m-%d")
                r = await client.get(f"{FMP_BASE}/economics-calendar",
                    params={"from": frm, "to": frm, "apikey": FMP_KEY})
...
async def diag_rapidapi(key: str = ""):
    ...
    params = {"from": today.strftime("%Y-%m-%d"),
              "to": (today + timedelta(days=1)).strftime("%Y-%m-%d"), "countries": "US"}
    paths = ["/", "/economic-calendar", "/economic_events", "/calendar",
             "/events", "/economic-events", "/api/economic-calendar"]
