...
async def refresh_heatmap_rest():
    ...
    try:
        ...
        if r.status_code != 200:
            print(f"[heatmap-rest] error {r.status_code}")
            return
        data = r.json()
        ...
        print(f"[heatmap-rest] ok: {len(data)} símbolos")
    except Exception as e:
        print(f"[heatmap-rest] error: {e}")
...
@app.get("/health")
def health():
    hm_count = len(cache["heatmap"]["data"])
    return {
        "flashalpha":           cache["health"]["flashalpha"],
        "twelvedata":           cache["health"]["twelvedata"],
        ...
        "gex_available":        bool(cache["gex"].get("NQ")),
    }
...
async def get_company(ticker: str):
    ...
    # Finnhub como fallback para company details
    if FINNHUB_KEY and not data.get("name"):
        try:
            ...
            if r.status_code == 200:
                p = r.json() or {}
                data = {**data, "name":p.get("name"), "sector":p.get("finnhubIndustry"),
