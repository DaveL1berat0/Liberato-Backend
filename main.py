...
async def _fetch_rapidapi_actuals(client):
    ...
    # NO lo enviamos y filtramos US en código. Pedimos high-impact ya resueltos
    # de los últimos 2 días (cubre el evento recién publicado + revisiones).
    params = {
        "daysBehind": "2", "daysAhead": "0",
        "impact": "High", "resolved": "true",
        "descriptions": "false",   # sin generación IA → respuesta más rápida
        "sort": "asc", "limit": "60", "tz": "America/New_York",
...
async def startup():
    ...
    # ── GEX al arrancar si el cache está FRÍO (post-deploy no se espera a las 7AM) ──
    # Un deploy fuera de la ventana 7-13 dejaba el dashboard sin datos reales
    # (y el frontend caía a placeholders). Costo: 1 refresh (~3-5 créditos).
    async def _gex_boot():
        try:
            g = cache["gex"].get("NQ", {}) or {}
            age = time.time() - (g.get("_ts") or 0)
            if age > 45 * 60:
                print(f"[startup] cache GEX frío ({age/60:.0f} min) → refresh inicial")
                await refresh_gex()
            else:
                print(f"[startup] cache GEX fresco ({age/60:.0f} min) — sin llamada")
        except Exception as e:
            ...
            if RAPIDAPI_KEY:
                ...
                r = await client.get(f"https://{RAPIDAPI_HOST}/calendar",
                    headers=headers, params={
                        "daysBehind": "2", "daysAhead": "0", "impact": "High",
                        "resolved": "true", "descriptions": "false", "limit": "60",
                        "tz": "America/New_York",
                        "fields": "id,date,eventName,country,impactLabel,actual,forecast,previous"})
